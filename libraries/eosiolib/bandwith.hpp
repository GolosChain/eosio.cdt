...
extern "C" {
    ...
    *  @return time
    */
   uint64_t get_bw_cpu_limit(eosio::name account);
   ...
   *  @return bytes
   */
   uint64_t get_bw_net_limit(eosio::name account);
   ...
   */
   bool is_provided_bw_confirmed(eosio::name account);


   /**
   *  @brief Set valuest of the banwith this contract is ready to provide
   */
   void set_bw_limits(eosio::name account, uint64_t net, uint64_t cpu);
